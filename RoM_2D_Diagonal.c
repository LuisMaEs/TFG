...
void Constructor1(float Clifford[8192][4], float Aux[4][4][2048], int n, int m);/*Aquí cortamos matriz a matriz*/
void Constructor2(float Aux[4][4][2048],float Final[32][2048]);/*Aquí dejamos ya la matriz en su forma final*/

float Aux[4][4][2048];
float Final[32][2048];
float Clifford[8192][4];
double valor[65536+1];
int fila[65536+1], columna[65536+1];
...
/*-------------------------------AQUI HAY QUE DEFINIR LA MATRIZ A DESCOMPONER. APLANADA, CLARO.------------------------------------------------*/
  ...
  glp_set_row_bnds(lp, 11, GLP_FX, cos(theta), cos(theta));
 ...
 /* PARTE IMAGINARIA DE LA MATRIZ ----------------------------------.*/
  ...
  glp_set_row_bnds(lp, 27, GLP_FX, sin(theta), sin(theta));
...
/*-------CREAR TODAS LAS VARIABLES NECESARIAS PARA RESOLVER EL PROBLEMA, VAN DE DOS EN DOS POR LAA DESCOMPOSICIÓN DEL VALOR ABSOLUTO------------------*/
  ...
  glp_add_cols(lp, 2048);/*Número de columnas que se añaden. Vamos, variables libres que tiene el problema*/
...
/*---CARGAR LAS MATRICES DEL GRUPO DE CLIFFORD (EN EL PROBLEMA LP) EN LAS QUE VAMOS A PROYECTAR LA MATRIZ OBJETIVO.-------------------------------*/
  ...
  glp_load_matrix(lp, 65536, fila, columna, valor);
  /* solve problem */
...
void Constructor1(float Clifford[8192][4], float Aux[4][4][2048], int n, int m)
{
  int i,j,k,l;
  for (i = 0; i < 2048; i++)
  {
...
void Constructor2(float Aux[4][4][2048],float Final[32][2048])
{
  int i,j,k,l,count;
  count=0;
  k=0;l=0;
